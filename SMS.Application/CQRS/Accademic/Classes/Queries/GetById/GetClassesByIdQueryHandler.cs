...
namespace SMS.Application.CQRS.Accademic.Classes.Queries.GetById
{
    public class GetClassesByIdQueryHandler(IClassesRepository repository, IMapper mapper)
        : IRequestHandler<GetClassesByIdQuery, ServiceResponse<ClassesDto>>
    {
        public async Task<ServiceResponse<ClassesDto>> Handle(GetClassesByIdQuery request, CancellationToken cancellationToken)
        {
            ...
            return ServiceResponse<ClassesDto>.Success(data: mapped);
        }
    }
}